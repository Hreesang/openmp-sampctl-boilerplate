...
public OnGameModeInit()
{
	...
	// Green classes
	AddPlayerClass(162,2117.0129,-224.4389,8.15,0.0,WEAPON_M4,100,WEAPON_MP5,200,WEAPON_SNIPER,10);
	AddPlayerClass(157,2148.6606,-224.3336,8.15,347.1396,WEAPON_M4,100,WEAPON_MP5,200,WEAPON_SNIPER,10);

 	// Blue classes
	AddPlayerClass(154,2352.9873,580.3051,7.7813,178.1424,WEAPON_M4,100,WEAPON_MP5,200,WEAPON_SNIPER,10);
	AddPlayerClass(138,2281.1504,567.6248,7.7813,163.7289,WEAPON_M4,100,WEAPON_MP5,200,WEAPON_SNIPER,10);
	...
	{   // Green OBJECTIVE REACHED.
	    ...
	    if(gGreenTimesCapped==CAPS_TO_WIN) {
			...
			gObjectiveReached = 1;	PlaySoundForAll(1185, 0.0, 0.0, 0.0);
 			SetTimer("ExitTheGameMode", 6000, false); // Set up a timer to exit this mode.
		} else {
...
public OnVehicleStreamIn(vehicleid, forplayerid)
{
    ...
    if(vehicleid == OBJECTIVE_VEHICLE_BLUE) {
        if(gTeam[forplayerid] == TEAM_GREEN) {
			SetVehicleParamsForPlayer(OBJECTIVE_VEHICLE_BLUE,forplayerid,true,true); // objective; locked
		}
		else if(gTeam[forplayerid] == TEAM_BLUE) {
		    SetVehicleParamsForPlayer(OBJECTIVE_VEHICLE_BLUE,forplayerid,true,false); // objective; unlocked
		}
	}
	else if(vehicleid == OBJECTIVE_VEHICLE_GREEN) {
        if(gTeam[forplayerid] == TEAM_BLUE) {
			SetVehicleParamsForPlayer(OBJECTIVE_VEHICLE_GREEN,forplayerid,true,true); // objective; locked
		}
		else if(gTeam[forplayerid] == TEAM_GREEN) {
		    SetVehicleParamsForPlayer(OBJECTIVE_VEHICLE_GREEN,forplayerid,true,false); // objective; unlocked
		}
	}
