...
public OnGameModeInit()
{
	...
	txtObsHelper = TextDrawCreate(
		20.0, 
		400.0,
		"Press ~b~~k~~PED_SPRINT~ ~w~to spawn~n~Press ~b~~k~~PED_FIREWEAPON~ ~w~to switch players"
	);
	TextDrawUseBox(txtObsHelper, false);
	TextDrawFont(txtObsHelper, TEXT_DRAW_FONT_2);
	TextDrawSetShadow(txtObsHelper, false);
    TextDrawSetOutline(txtObsHelper, 1);
    TextDrawBackgroundColour(txtObsHelper, 0x000000FF);
    TextDrawColour(txtObsHelper, 0xFFFFFFFF);
